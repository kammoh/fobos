...
def printHexMatrix(A, printAll=False):
    import sys
    lim = 10
...
def main():
    ...
    cpa = CPA()
    #V = cpa.genIntermediateMatrix(D, K, exampleMappingFunction1)
    #print("D=")
    #printHexMatrix(D)
    #print("K=")
    #printHexMatrix(K)
    ...
    #A = loadTextMatrix('./example_data/ciphertexts1.txt')
    #print("A=")
    #printHexMatrix(A)
    ####test aes sbox fiirst round
    #plaintext = loadTextMatrix('./example_data/ciphertexts1.txt')
    plaintext = loadTextMatrix('./plaintext1.txt', 3)
    print("plaintext=")
    printHexMatrix(plaintext)
    keys = loadTextMatrix('./example_data/keys1.txt')
    print (plaintext)
    numPlainTexts = plaintext.shape[0]
    res = AESSboxOutFirstRound(plaintext[:, 0].reshape(numPlainTexts,1))
    print("RES=")
    printHexMatrix(res)
