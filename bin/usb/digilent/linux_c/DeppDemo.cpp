...
void DoPutReg() {
	...
	idReg = (BYTE) strtol(szRegister, &szStop, 16);
	idData = (BYTE) strtol(szByte, &szStop, 16);
...
void DoPutRegRepeat() {
	...
	BYTE	idReg;
	char *	szStop;
        BYTE rgbLd1[cbBlockSize];
        BYTE    rgbLd[cbBlockSize];
	int		rv, cbSend, cbSendTotal, cbSendCheck, i;
    ...
    printf ("Bytes to Send = %ld\n cbsendtotal = %d\n ", cb, cbSendTotal);
		...
		printf ("i = %d\n", i);
            for (i=0;i < cb; i++) {printf ("i = %d\n", i);
                 rv = fscanf(fhin, "%x", &rgbLd1[i]);
		...
		if(!DeppPutRegRepeat(hif, idReg, rgbLd1, cbSend, fFalse)){
