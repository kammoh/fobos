...
class TVLACalc:
    ...
    def addTrace(self, trace, setNum):
        if setNum == 0:
            ...
            self.CS3_0 = (
                self.CS3_0
                - 3 * d_0 / n_0 * self.CS2_0
                + d_0**3 * (n_0 - 1) * ((n_0 - 1) ** 2 - 1) / n_0**3
            )
            ###CS4
            self.CS4_0 = (
                self.CS4_0
                - 4 * d_0 / n_0 * self.CS3_0
                + 6 * d_0**2 / n_0**2 * self.CS2_0
                + d_0**4 * (n_0 - 1) * ((n_0 - 1) ** 3 + 1) / n_0**4
            )
            ...
            self.CS3_1 = (
                self.CS3_1
                - 3 * d_1 / n_1 * self.CS2_1
                + d_1**3 * (n_1 - 1) * ((n_1 - 1) ** 2 - 1) / n_1**3
            )
            ###CS4
            self.CS4_1 = (
                self.CS4_1
                - 4 * d_1 / n_1 * self.CS3_1
                + 6 * d_1**2 / n_1**2 * self.CS2_1
                + d_1**4 * (n_1 - 1) * ((n_1 - 1) ** 3 + 1) / n_1**4
            )
    ...
    def calcTvalues1(self):
        ...
        var0 = self.CM2_0
        var1 = self.CM2_1
        t_array = (m0 - m1) / np.sqrt(
            (var0 + epsilon) / self.n_0 + (var1 + epsilon) / self.n_1
        )  # div by zero issue
...
if __name__ == "__main__":
    ...
    tCalc.doTVLA(
        traceFile="fobosworkspace/aes_tvla/capture/attempt-01/powerTraces.npy",  # os.path.join(captureDir, acqConf['traceFile']),
