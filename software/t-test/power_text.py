...
ostr = "Text Format Power Computer"
print ostr
...
ostr = "Computing average power at each sample"
print ostr
  
for j in range (start_sample, end_sample):
    Vaccum = 0 # clear accumulator 
    for i in range (0,num_of_traces-1):
      Vaccum += raw_traces0[i,j]
    Vaccum = Vaccum / num_of_traces  # average  
    ostr =  str(computePower(Vaccum))
    ostr = ostr + '\n'
    writeFile.write(ostr)

ostr = "Computation complete."
print ostr
