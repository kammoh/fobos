...
#Date : Jan 2018
##########GENERAL SETTINGS
start_ylim  = 0.95 #Plot ymlim range
end_ylim    = 1.13
##########END GENERAL SETTINGS
###########CLK GRAPH SETTINGS
display_clk = "YES"# 'YES' | 'NO' Enable/Disable clock plotting
num_of_clks = 739   #num of clocks to in trace
clk_high    = 1.11   #high clock voltage for plotting
clk_low     = 1.05  #low clock voltage for plotting
...
if display_clk == "YES":
   ...
   for n in range(0, num_of_clks * 2):
      if(n%2 ==0):
         ...
         if n/2 < len(states):
            plt.text(x_offset, clk_high +  clk_high*.001 , "S" + str(states[n/2]), rotation = 90)  #print states on the plot
